...
class MentalModel(BaseModel):
    ...
    unknowns: List[str] = Field(default_factory=list, description="Major systemic blockers preventing a decision (not last question asked)")
...
class DestinationCandidate(BaseModel):
    ...
    decision_criteria: Optional[dict] = Field(None, description="Key unknowns/deciding factors specific to this destination")
...
class VacationPlan(BaseModel):
    ...
    candidates: List[DestinationCandidate] = Field(default_factory=list, description="Persistent ledger of destination candidates")
    budget_range: Optional[str] = Field(None, description="Budget range (e.g. '$2000-$3000')")
    comparison_matrix: Optional[List[dict]] = Field(None, description="MCDM trade-off rows — populated in Compare phase")
    notes: str = Field("", description="Free text notes")


# --- Patch Models (for update_plan tool) ---
# Note: candidates are managed exclusively via manage_candidates tool, not this patch.
...
class VacationPlanPatch(BaseModel):
    ...
    Does NOT include candidates or comparison_matrix — those are managed by dedicated tools.
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Phase value:", plan.phase.value)
        candidate = DestinationCandidate(name="Lisbon", status="active", rationale="Great weather and budget-friendly")
        plan.candidates.append(candidate)
        print("Candidates:", [c.name for c in plan.candidates])
