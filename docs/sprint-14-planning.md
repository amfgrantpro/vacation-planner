...
* **Dedicated generation endpoints handle AI tasks.** Each endpoint does one job: read what it needs from DB, call LLM, write result to DB, return. The agent no longer manages the visual space.  
...
1. Planning is complete. The PM has approved the Sprint 14 plan.
2. The `Sprint 14 Spec` has been written and approved: `sprint-14-spec.md`.  
